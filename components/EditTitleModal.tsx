...
export default function EditTitleModal({ title, emoji, onClose, onSubmit }: Props) {
  ...
  const handleSubmit = () => {
    if (!newTitle.trim()) {
      setError("Split name required");
      return;
    }
    onSubmit(newTitle.trim(), newEmoji);
  };
    ...
    <>
      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center px-4">
        ...
        >
          ...
          <h3 className="font-syne text-lg font-bold text-white">Edit Split</h3>
          <motion.button
            whileTap={{ scale: 0.88 }}
            onClick={onClose}
