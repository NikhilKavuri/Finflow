...
export default function ProfilePage() {
  ...
  const handleAddBank = () => {
    ...
    const balance = parseFloat(bankBalance) || undefined;
  ...
  const handleUpdateBank = () => {
    ...
    const balance = parseFloat(bankBalance) || undefined;
    updateBank(editingBank.id, { name: bankName.trim(), balance });
    setEditingBank(null);
    setBankName("");
    setBankBalance("");
  };
  ...
  const handleDeleteBank = (id: string) => {
    if (state.banks.length <= 1) return; // Prevent deleting the last bank
    deleteBank(id);
  };
              ...
              <div className="space-y-3">
                ...
                <div className="flex gap-2">
                  <motion.button
                    ...
                    className="flex-1 py-2 rounded-lg font-semibold text-white bg-[#6c47ff] transition-colors"
                  >
